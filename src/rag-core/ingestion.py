...
from ingestion_utils import (
    S3Repository, VectorDBRepository, get_embeddings
)
...
def _build_retriever() -> ParentDocumentRetriever:
    ...
    sql_store = SQLStore(
        db_url=env.DB_URL, 
        namespace=f"{env.COLLECTION_NAME}_parents"
    )
    ...
    return ParentDocumentRetriever(
        ...
        child_splitter=RecursiveCharacterTextSplitter(
            chunk_size=env.CHUNK_SIZE_C, chunk_overlap=env.CHUNK_OVERLAP_C, separators=["\n\n", "\n", " "]
        ),
        parent_splitter=RecursiveCharacterTextSplitter(
            chunk_size=env.CHUNK_SIZE_P, chunk_overlap=env.CHUNK_OVERLAP_P, separators=["\n#", "\n##", "\n\n\n"]
        ),
    )
